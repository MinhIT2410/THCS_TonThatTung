...
import { X, AlertCircle, RefreshCw, Save, Image as ImageIcon } from 'lucide-react';
...
export default function GalleryBlockEditor({
  ...
  const [formData, setFormData] = useState({
    eyebrow: overrideData?.eyebrow ?? defaultData?.eyebrow ?? 'Ghi dấu kỷ niệm',
    title: overrideData?.title ?? defaultData?.title ?? 'Khoảnh khắc đẹp Liên đội',
    description: overrideData?.description ?? defaultData?.description ?? 'Ghi lại những khoảnh khắc tươi đẹp đầy sức trẻ dưới mái trường của chúng mình.',
    buttonLabel: overrideData?.buttonLabel ?? defaultData?.buttonLabel ?? 'Mở thư viện ảnh',
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error('Failed to save gallery block config:', err);
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-1">
              ...
              <input
                ...
                className="w-full text-xs rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 p-2.5 text-slate-800 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-amber-500/30"
                placeholder="Ví dụ: Ghi dấu kỷ niệm"
                required
              />
            </div>
            ...
            <div className="space-y-1">
              ...
              <input
                ...
                className="w-full text-xs rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 p-2.5 text-slate-800 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-amber-500/30"
                placeholder="Ví dụ: Khoảnh khắc đẹp Liên đội"
                required
              />
            </div>
            ...
            <div className="space-y-1">
              ...
              <input
                ...
                className="w-full text-xs rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 p-2.5 text-slate-800 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-amber-500/30"
                placeholder="Ví dụ: Mở thư viện ảnh"
                required
              />
            </div>
          </div>
          ...
          <div className="flex items-center justify-end space-x-3 pt-4 border-t border-slate-100 dark:border-slate-850">
            ...
            <button
              type="submit"
              disabled={isSaving}
              className="flex items-center space-x-1.5 rounded-xl bg-amber-600 hover:bg-amber-700 disabled:bg-amber-800 px-5 py-2.5 text-xs font-bold text-white shadow-lg active:scale-[0.98] transition-all cursor-pointer"
            >
              {isSaving ? (
