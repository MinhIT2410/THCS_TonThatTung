...
import { X, AlertCircle, RefreshCw, Save, Newspaper } from 'lucide-react';
...
export default function NewsBlockEditor({
  ...
  const [formData, setFormData] = useState({
    eyebrow: overrideData?.eyebrow ?? defaultData?.eyebrow ?? 'Măng non tin nhanh',
    title: overrideData?.title ?? defaultData?.title ?? 'Tin tức măng non',
    description: overrideData?.description ?? defaultData?.description ?? 'Cập nhật tin hoạt động Đội, gương sáng thiếu nhi và thông báo mới nhất.',
    buttonLabel: overrideData?.buttonLabel ?? defaultData?.buttonLabel ?? 'Xem tất cả tin tức',
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error('Failed to save news block config:', err);
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-1">
              ...
              <input
                ...
                placeholder="Ví dụ: Măng non tin nhanh"
                required
              />
            </div>
            ...
            <div className="space-y-1">
              ...
              <input
                ...
                className="w-full text-xs rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 p-2.5 text-slate-800 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-red-500/30"
                placeholder="Ví dụ: Tin tức măng non"
                required
              />
            </div>
            ...
            <div className="space-y-1">
              ...
              <input
                ...
                className="w-full text-xs rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 p-2.5 text-slate-800 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-red-500/30"
                placeholder="Ví dụ: Xem tất cả tin tức"
                required
              />
            </div>
          </div>
