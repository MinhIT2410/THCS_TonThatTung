...
import { X, AlertCircle, RefreshCw, Save, Sparkles } from 'lucide-react';

interface CampaignsBlockEditorProps {
  title: string;
  pageKey: string;
  blockKey: string;
  defaultData: any;
...
export default function CampaignsBlockEditor({
  title,
  defaultData,
  overrideData,
  onClose,
  onSave,
}: CampaignsBlockEditorProps) {
  const [formData, setFormData] = useState({
    eyebrow: overrideData?.eyebrow ?? defaultData?.eyebrow ?? 'Phong trào sôi nổi',
    title: overrideData?.title ?? defaultData?.title ?? 'Phong trào thi đua đang diễn ra',
    description: overrideData?.description ?? defaultData?.description ?? 'Các bạn hãy tham gia tích cực để cùng hoàn thành phong trào Đội viên tốt nhé.',
    buttonLabel: overrideData?.buttonLabel ?? defaultData?.buttonLabel ?? 'Xem toàn bộ kế hoạch',
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error('Failed to save campaigns block config:', err);
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-1">
              ...
              <input
                ...
                placeholder="Ví dụ: Phong trào sôi nổi"
                required
              />
            </div>
            ...
            <div className="space-y-1">
              ...
              <input
                ...
                className="w-full text-xs rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 p-2.5 text-slate-800 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-blue-500/30"
                placeholder="Ví dụ: Phong trào thi đua đang diễn ra"
                required
              />
            </div>
            ...
            <div className="space-y-1">
              ...
              <input
                ...
                className="w-full text-xs rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 p-2.5 text-slate-800 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-blue-500/30"
                placeholder="Ví dụ: Xem toàn bộ kế hoạch"
                required
              />
            </div>
          </div>
