...
export default function AdminAlbumsPage() {
  ...
  return (
    <div className="space-y-6 py-4 font-sans" id="admin-albums-page">
      {/* Page Header */}
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-slate-200 dark:border-slate-800 pb-5">
        <div className="space-y-1">
          <div className="flex items-center space-x-2.5">
            <ImageIcon className="h-6 w-6 text-rose-600 dark:text-rose-400" />
            <h1 className="font-display text-xl font-bold text-slate-900 dark:text-white">Quản lý album ảnh</h1>
          </div>
          <p className="text-xs text-slate-500 dark:text-slate-400">Tạo mới, quản lý hình ảnh thư viện và hoạt động ngoại khóa nhà trường.</p>
        </div>
        <button
      ...
      <div className="bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-3xl p-8 sm:p-12 text-center max-w-2xl mx-auto space-y-6">
        <div className="p-4 bg-rose-50 dark:bg-rose-950/20 text-rose-600 dark:text-rose-400 rounded-2xl w-16 h-16 flex items-center justify-center mx-auto shadow-sm border border-rose-100/40 dark:border-rose-900/30">
        ...
        <div className="space-y-2">
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400 max-w-md mx-auto leading-relaxed">
            Tạo album, upload ảnh, sắp xếp ảnh sẽ được bổ sung ở Prompt 13. Hãy quay lại sau khi tính năng này được cập nhật đầy đủ.
          </p>
        </div>
